...
begin
  if to_regclass('public.contracts') is null then
    raise notice 'Skipping contract milestones migration: table public.contracts does not exist';
    return;
  end if;
  ...
  if to_regclass('public.project_progress_reports') is not null then
    ...
    raise notice 'Skipping project_progress_reports contract linkage: table public.project_progress_reports does not exist';
  end if;
  ...
  if to_regclass('public.project_progress_items') is not null then
    ...
    alter table public.project_progress_items
      add column if not exists contract_milestone_id uuid references public.contract_milestones (id) on delete restrict;
    ...
    alter table public.project_progress_items drop constraint if exists project_progress_items_unit_price_nonneg;
    alter table public.project_progress_items
    ...
    raise notice 'Skipping project_progress_items contract linkage: table public.project_progress_items does not exist';
  end if;
end
$$;
