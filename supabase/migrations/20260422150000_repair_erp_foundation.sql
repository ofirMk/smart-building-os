-- REPAIR: erp_companies + erp_md_* foundation tables.
-- Migration 20260419130000 and 20260626120000 were tracked as applied but their DDL
-- never executed on the remote (tables do not exist). This repair migration re-creates
-- the full foundation using IF NOT EXISTS so it is safe to run even if partially applied.
-- Timestamp chosen to run just before the scaffold migrations that depend on erp_companies.
...
create policy erp_companies_all_authenticated
  on public.erp_companies for all to authenticated
