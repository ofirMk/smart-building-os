/** RTL loading shell for lazy-loaded Executive BI (matches Jimmy Standard: bg-white, slate borders). */
  ...
  return (
    ...
    >
      ...
      <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-2">
        <div className="min-h-[240px] animate-pulse rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 p-3">
            <div className="h-4 w-40 rounded bg-slate-100" />
          </div>
          <div className="space-y-2 p-3">
            {Array.from({ length: 5 }).map((_, j) => (
              <div key={j} className="h-8 w-full rounded bg-slate-50" />
            ))}
          </div>
        </div>
        <div className="min-h-[240px] animate-pulse rounded-lg border border-slate-200 bg-white shadow-sm">
          ...
          <div className="space-y-2 p-3">
            {Array.from({ length: 4 }).map((_, j) => (
              <div key={j} className="h-14 w-full rounded bg-slate-50" />
            ))}
          </div>
        </div>
      </div>
