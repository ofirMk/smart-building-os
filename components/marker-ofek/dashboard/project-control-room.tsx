...
export function ProjectControlRoom() {
  return (
    <div
      dir="rtl"
      lang="he"
      className="flex min-h-0 min-w-0 flex-1 flex-col gap-4 bg-background p-3 text-foreground md:p-4"
    >
      {/* Ribbon */}
      ...
      <div className="grid min-h-0 flex-1 gap-4 md:grid-cols-2">
        <section className="flex min-h-[220px] flex-col rounded-xl border border-border bg-card p-4 text-card-foreground shadow-sm">
          <h2 className="mb-3 flex items-center gap-2 border-b border-border pb-2 text-sm font-bold text-foreground">
          ...
          <ul className="flex flex-1 flex-col gap-2 overflow-auto text-sm">
            {MOCK_RECENT_LOGS.map((log) => (
              <li
                ...
                className="rounded-md border border-border bg-muted/35 px-3 py-2"
              >
                <p className="text-xs font-semibold text-muted-foreground">{log.date}</p>
...
function KpiCard({
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 text-card-foreground shadow-sm hover-effect transition-all duration-200 hover:shadow-md active:scale-[0.98]">
