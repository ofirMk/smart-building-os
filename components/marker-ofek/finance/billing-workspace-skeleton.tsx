  ...
  return (
    ...
    >
      <div className="border-b border-slate-200 px-4 py-3 md:px-6">
        ...
        <div className="mt-2 h-3 w-full max-w-lg animate-pulse rounded bg-slate-50" />
          ...
          <div className="border-b border-slate-100 p-2">
            <div className="flex gap-2">
              {Array.from({ length: 6 }).map((_, c) => (
                <div
                  key={c}
                  className="h-6 flex-1 animate-pulse rounded bg-slate-50"
                />
              ))}
            </div>
          </div>
            ...
            {Array.from({ length: 8 }).map((_, r) => (
              <div key={r} className="flex gap-2">
                {Array.from({ length: 6 }).map((_, c) => (
                  <div
                    key={c}
                    className="h-9 flex-1 animate-pulse rounded border border-slate-50 bg-slate-50/80"
                  />
                ))}
              </div>
            ))}
