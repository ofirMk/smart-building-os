import type { ExecutiveHealthReport } from "@/lib/infrastructure/system-health-types"
...
/**
 * תבנית HTML לדוחות מערכת — עיצוב נקי ("Pharmacy clean"), תואם לקליינטי מייל.
 */
...
<head>
  ...
  <title>Diamond System Report</title>
</head>
    ...
    <tr>
      <td align="center">
        <table role="presentation" width="100%" style="max-width:640px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(15,23,42,0.08);">
          <tr>
            <td style="background:linear-gradient(135deg,${brand.accent} 0%,#0f766e 100%);padding:28px 24px;color:#fff;">
              <div style="font-size:11px;letter-spacing:0.12em;text-transform:uppercase;opacity:0.9;">Executive summary</div>
              <h1 style="margin:8px 0 0;font-size:22px;font-weight:700;line-height:1.25;">דוח בריאות מערכת — ${brand.name}</h1>
          ...
          <tr>
            <td style="padding:20px 24px 28px;">
              <p style="color:${brand.muted};font-size:12px;line-height:1.5;margin:0 0 12px;">
                דוח אוטומטי — נשלח לכתובת התמיכה של הארגון. אין להשיב למייל זה.
              </p>
